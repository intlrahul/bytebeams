...
**Resolved decisions:** Use a dedicated map-free Geofences route with persisted list, add/edit forms, and deactivation only; never hard-delete a geofence. Validate a trimmed non-empty 1–80-character name while retaining its entered display form, radius 50 m–50 km inclusive, latitude -90…90 inclusive, and longitude -180…180 inclusive. Seed five synthetic demo-labelled Bengaluru operational sites: Sarjapur Hub, Electronic City Depot, Whitefield Service Yard, Peenya Logistics Hub, and Yelahanka Charging Yard. Rebuild each affected vehicle from its complete retained location history. Transition identity is `vehicle_id + geofence_id + geofence_version + transition_type + event_timestamp + packet_id`. The backend persists a looping twelve-tick scenario for five showcase vehicles covering approach, entry, dwell, exit, same-origin return, site-to-site movement, boundary noise, and rejected accuracy; mobile derives all membership and transitions from those raw packets. Showcase telemetry synchronizes location, speed, ignition, connectivity, SOC, range, odometer, and battery temperature. Moving consumes 0.2 SOC points per second, charging adds 0.5 within a 10–90% bound, and range uses `range_km = SOC × 3`.

**Validation evidence:** Server formatting, lint, build, and all 30 tests passed with 93.75% line coverage. Flutter analysis and all 191 unit/widget tests passed with 93.20% line coverage. Android integration tests passed for durable geofence membership, live entry/exit projection, and synchronized Vehicle Detail Speed, SOC, and Range updates.
